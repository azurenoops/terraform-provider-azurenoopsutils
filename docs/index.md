...
page_title: "Azure NoOp Accelerator Provider"
subcategory: ""
description: |-
  This provider implements a set of methodologies for naming convention implementation for Azure.
---

# Azure NoOp Accelerator Provider

The Azure NoOp Accelerator Provider is a *logical provider* which means that it works entirely within Terraform's logic, and doesn't interact with any other services. The goal of this provider is to provide helper methods in using Azure NoOp Accelerator using Terraform.

The AzureNoOps provider currently contains a one resource based on the Terraform Random_string provider. The naming_convention resources enforce is the first iteration of our provider implementation enforcing an consistant naming convention.

As per the growing number of azure resources a new implementation is now available using the azurenoopsutils_resource_name resource to avoid breaking changes. The new implementation supports an extensive list of resource types and will be updated on a regular basis as new services are released.
